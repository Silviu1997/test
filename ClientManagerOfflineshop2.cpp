	...
	//for (size_t j = 1; j <= channelcount; j++)
	{
		...
		if (pMsg && pMsg->Get()->uiNumRows != 0)
		{
			while (NULL != (row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
			{
				...
				snprintf(szQuery, sizeof(szQuery), "SELECT id,owner_id,pos,count,vnum,socket0,socket1,socket2,socket3,socket4"
				...
				",price,status,ownername,buyername"
#ifdef ENABLE_CHANGELOOK_SYSTEM
				",transmutation"
#endif
				" FROM player.offline_shop_item WHERE owner_id = %u", offlineshop->owner_id);
...
void CClientManager::CreateOfflineShop(TOfflineShop* p)
{
	...
	for (DWORD i = 0; i < OFFLINE_SHOP_HOST_ITEM_MAX_NUM; ++i)
	{
		...
		iUpdateLen += snprintf(szValues + iUpdateLen, sizeof(szValues) - iUpdateLen, ",%ld,%ld,%ld,%ld,%ld", offlineshop->items[i].alSockets[0], offlineshop->items[i].alSockets[1], offlineshop->items[i].alSockets[2], offlineshop->items[i].alSockets[3], offlineshop->items[i].alSockets[4]);
		...
		iUpdateLen += snprintf(szValues + iUpdateLen, sizeof(szValues) - iUpdateLen, ",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
			offlineshop->items[i].aAttr[0].bType, offlineshop->items[i].aAttr[0].sValue,
			offlineshop->items[i].aAttr[1].bType, offlineshop->items[i].aAttr[1].sValue,
			offlineshop->items[i].aAttr[2].bType, offlineshop->items[i].aAttr[2].sValue,
			offlineshop->items[i].aAttr[3].bType, offlineshop->items[i].aAttr[3].sValue,
			offlineshop->items[i].aAttr[4].bType, offlineshop->items[i].aAttr[4].sValue,
			offlineshop->items[i].aAttr[5].bType, offlineshop->items[i].aAttr[5].sValue,
			offlineshop->items[i].aAttr[6].bType, offlineshop->items[i].aAttr[6].sValue);
...
void CClientManager::AddItem(OFFLINE_SHOP_ITEM* item)
{
	...
	iLen += snprintf(szColumns + iLen, sizeof(szColumns) - iLen, ",socket0,socket1,socket2,socket3,socket4");
	iUpdateLen += snprintf(szValues + iUpdateLen, sizeof(szValues) - iUpdateLen, ",%ld,%ld,%ld,%ld,%ld", newitem.alSockets[0], newitem.alSockets[1], newitem.alSockets[2], newitem.alSockets[3], newitem.alSockets[4]);

	iLen += snprintf(szColumns + iLen, sizeof(szColumns) - iLen, ", attrtype0, attrvalue0, attrtype1, attrvalue1, attrtype2, attrvalue2, attrtype3, attrvalue3, attrtype4, attrvalue4, attrtype5, attrvalue5, attrtype6, attrvalue6");
	iUpdateLen += snprintf(szValues + iUpdateLen, sizeof(szValues) - iUpdateLen, ",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
		newitem.aAttr[0].bType, newitem.aAttr[0].sValue,
		newitem.aAttr[1].bType, newitem.aAttr[1].sValue,
		newitem.aAttr[2].bType, newitem.aAttr[2].sValue,
		newitem.aAttr[3].bType, newitem.aAttr[3].sValue,
		newitem.aAttr[4].bType, newitem.aAttr[4].sValue,
		newitem.aAttr[5].bType, newitem.aAttr[5].sValue,
		newitem.aAttr[6].bType, newitem.aAttr[6].sValue);
#ifdef ENABLE_CHANGELOOK_SYSTEM
	iLen += snprintf(szColumns + iLen, sizeof(szColumns) - iLen, ",transmutation");
...
void CClientManager::BuyItem(TOfflineShopBuy* buyItem)
{
	...
	for (DWORD i = 0; i < OFFLINE_SHOP_HOST_ITEM_MAX_NUM; i++)
	{
		...
		snprintf(szQuery, sizeof(szQuery), "INSERT INTO player.offline_shop_log ("
		...
		",attrtype0, attrvalue0, attrtype1, attrvalue1, attrtype2, attrvalue2, attrtype3, attrvalue3, attrtype4, attrvalue4, attrtype5, attrvalue5, attrtype6, attrvalue6) VALUES("
		"%u, '%s', '%s', %d, %d, %lld"
		",%u, %d"
#ifdef ENABLE_CHANGELOOK_SYSTEM
		",%d"
#endif
		",%ld, %ld, %ld, %ld, %ld"
		",%d, %ld,%d, %ld,%d, %ld,%d, %ld,%d, %ld,%d, %ld,%d, %ld)",
		item.owner_id, buyItem->customer_name, buf, item.vnum, item.count, item.price,
		item.id, item.pos
#ifdef ENABLE_CHANGELOOK_SYSTEM
		,item.transmutation
#endif
		, item.alSockets[0], item.alSockets[1], item.alSockets[2], item.alSockets[3], item.alSockets[4]
